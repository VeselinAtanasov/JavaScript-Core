function robot() {
    ...
    function interpretCommand(tokens) {
        let command = tokens[0];

        let response = "";
        switch (command) {
            case "report":
                response = reportStatistics();
                return response;
            case "restock":
                response = restockCommand(tokens)
                return response;
            case "prepare":
                response = prepareRecipe(tokens)
                return response;
            default:
                return;
        }
    ...
    return function (command) {
        ...
        let result = interpretCommand(tokens);
        return result
    }
}