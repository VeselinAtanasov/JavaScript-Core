function fillFormOrigin(username, email, phone, data) {
...
function fillFormImproved(username, email, phone, data) {
    ...
    data.forEach(line => {
        line = line.replace(/<([!@+])[a-zA-Z]+([!@+])>/g, rpl); //here when we calling the function we are not passing the params reason:
